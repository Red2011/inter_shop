...
export const result = (id: number): string => {
    if (id === 29) {
        return 'object-top'
    }
    return 'object-left'
...
export default function MainPage (): JSX.Element {
    ...
    if (error === true) return <p className=" h-screen w-screen bg-red-700 text-black text-xl flex justify-center items-center">ERROR</p>
    const block = (): JSX.Element[] | ReactElement => {
        if (data == null) { return <p className="h-screen w-screen text-black text-xl flex justify-center ">Loading...</p> } else {
            return (
                data.map((item) => (
                    <div key={item.id}
                        className="max-[590px]:w-[150px] max-[590px]:h-auto m-2 p-2 flex flex-col justify-between items-center bg-white/20 hover:bg-white/40 rounded-2xl">
                        <div className="flex flex-col justify-center max-[590px]:w-[130px]">
                            <div className="flex justify-center h-full w-full">
                                <Link className="max-[590px]:w-[115px]" href={`/products/${item.id}`}>
                                    <LazyLoadImage
                                        className={`w-[200px] h-[160px] object-cover ${result(item.id)} rounded-full max-[590px]:w-[115px] max-[590px]:h-[60px] `}
                                        src={item.thumbnail} effect="blur" alt={`product ${item.title}`}
                                    />
                                </Link>
                            </div>
                            <div
                                className="w-[210px] h-full my-1 text-center break-all max-[590px]:w-[130px] max-[590px]:text-[10px]">
                                {item.title}
                            </div>
                        </div>
                        <div className="flex justify-center">
                            <div
                                className="inline-block bg-blue-500 rounded-full p-2 shadow-lg shadow-black/40 max-[590px]:text-[8px]">Price: {item.price}$
                            </div>
                        </div>
                    </div>
                ))
